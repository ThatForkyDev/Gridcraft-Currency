...
public class CurrencyTakeCommand {
    @ForeignCommand(value = "take", permission = "command.currency.take", description = "Take a players balance", usage = "take <player> <amount>")
    public void take(CommandArgument argument) {
        ...
        if (argument.getArgs().length == 3) {
            ...
            if (TextUtil.isDouble(argument.getArgs()[2], true)) {
                ...
                if (account.isPresent()) {
                    account.get().takeBalance(amount);
                    sender.sendMessage(ChatColor.GREEN + "You have taken $" + TextUtil.decimalFormat(amount) + " from " + target.getName() + "'s balance.");
                } else {
                    if (CurrencyDAO.takeBalance(BaseDatabase.getInstance().getConnection(), uuid, amount)) {
                        sender.sendMessage(ChatColor.GREEN + "You have taken $" + TextUtil.decimalFormat(amount) + " from " + target.getName() + "'s balance.");
                    } else {
